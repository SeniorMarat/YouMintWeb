...
import { Address, beginCell, beginDict, Cell, contractAddress, StateInit } from "ton"
...
export async function deployJetton({
  ...
  const minterCode = Cell.fromBoc(compiledMinter.hex)[0]
  const walletCode = Cell.fromBoc(compiledWallet.hex)[0]

  const data = beginCell()
    .storeCoins(0)
    .storeAddress(adminAddress)
    .storeRef(buildJettonOnchainMetadata(metadata))
    .storeRef(walletCode)
    .endCell()

  const address = contractAddress({ workchain: 0, initialCode: minterCode, initialData: data })
  const cell = new Cell()
  new StateInit({ data, code: minterCode }).writeTo(cell)

  return {
    address: address.toFriendly({ testOnly: true }),
